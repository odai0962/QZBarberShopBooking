...
namespace QZBarberShopBooking.Service.Users;

public class UserService : IUserProfileService, IUserAdminService, IScopedService
{
    ...
    public async Task<UserDto> CreateAsync(CreateUserDto createUserDto, CancellationToken cancellationToken = default)
    {
        ...
        switch (role.Name)
        {
            ...
            case "Employee":
                ...
                break;
            case "Admin":
                var admin = _mapper.Map<Admin>(createUserDto);
                admin.PasswordHash = passwordHash;
                admin.RoleId = role.Id;
                admin.CreationDate = DateTime.UtcNow;
                admin.IsActive = createUserDto.IsActive;
                await _adminRepository.InsertAsync(admin, cancellationToken);
                createdUser = admin;
                break;
            default:
                throw new ValidationException(new Dictionary<string, string[]>
    ...
    public async Task<UserProfileDto> UpdateProfileAsync(int userId, UpdateProfileDto updateProfileDto, CancellationToken cancellationToken = default)
    {
        ...
        // AutoMapper's Map<TSource,TDestination> overload resolves TDestination from the
        // compile-time type of `user` (User), not its runtime type, so a Customer-only field
        // like DateOfBirth needs the Customer map picked explicitly here.
        if (user is Customer customer)
            _mapper.Map(updateProfileDto, customer);
        else
