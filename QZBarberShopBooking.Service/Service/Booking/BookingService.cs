...
public class BookingService : IBookingService, IScopedService
{
    ...
    public async Task<BookingDto> CreateAsync(CreateBookingDto createBookingDto, int customerId)
    {
        ...
        await _unitOfWork.ExecuteInTransactionAsync(async ct =>
        {
            ...
            {
                booking.Services.Add(new Domain.Entities.BookingService
                {
    ...
    public async Task<bool> CancelAsync(int id, int userId)
    {
        ...
        if (booking.CustomerId != userId && booking.EmployeeId != userId)
    ...
    public async Task<bool> ConfirmAsync(int id, int employeeId)
    {
        return await UpdateStatusForEmployeeAsync(id, employeeId, BookingStatus.Confirmed);
    }

    public async Task<bool> CompleteAsync(int id, int employeeId)
    {
        return await UpdateStatusForEmployeeAsync(id, employeeId, BookingStatus.Completed);
    }
    ...
    private async Task<bool> UpdateStatusForEmployeeAsync(int id, int employeeId, BookingStatus status)
    {
        var booking = await _bookingRepository.GetAll()
            .FirstOrDefaultAsync(b => b.Id == id && b.EmployeeId == employeeId && !b.IsDeleted)
            ?? throw new NotFoundException("Booking", id);
